...
# 这个类负责收集数据，存储在内存中，并在需要时写入磁盘。它支持增量收集和移动检查，以确保只保存有意义的数据。
# KEY_BANNED 列表中的键在移动检查中被忽略，因为它们不代表实际的物理状态变化（例如时间戳）。
# merge_config 函数用于合并两个配置字典，处理嵌套结构，并记录任何冲突的键值对。
# move_check_success 用于静止数据帧跳过
# resume 参数控制是否在初始化时自动计算下一个可用的 episode index，以便在继续之前的收集过程中避免覆盖现有数据。
# add_extra_cfg_info 用于在写入磁盘前添加额外的配置信息，例如环境参数等。
class CollectAny:
    def __init__(self, config = None, start_episode = 0, resume = False):
        ...
        self.skip_silent_frame = config.get("skip_silent_frame", False) if config is not None else False
    ...
    def _get_next_episode_index(self):
        """
        Determines the next available episode index for data collection.

        This method scans the save directory for existing HDF5 files and finds the next
        available episode ID that hasn't been used yet. It uses a sequential numbering
        scheme starting from 0.

        The process:
        1. Constructs the save directory path from configuration parameters
        2. Returns 0 if the directory doesn't exist (first episode)
        3. Collects all existing HDF5 file IDs from the directory
        4. Finds the first integer that is not in the existing IDs set
        5. Logs the result for debugging purposes

        Returns:
            int: The next available episode index. Returns 0 if no episodes exist yet,
                 otherwise returns the smallest non-negative integer not currently used
                 by existing episodes.

        Example:
            If episodes 0, 1, 2 exist, returns 3.
            If episodes 0, 2, 3 exist, returns 1.
            If no episodes exist, returns 0.
        """
        ...
        if self.skip_silent_frame:
            if controllers_data is None:
                ...
                if self.move_check_success(controllers_data, tolerance=self.move_tolerance):
                    self.episode.append(episode_data)
                    self.last_controller_data = controllers_data
                    debug_print("CollectAny", f"robot is moving, collect this frame!", "DEBUG")
                else:
