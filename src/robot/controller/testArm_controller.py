...
class TestArmController(ArmController):
    ...
    def set_position(self, position):
        if position.shape[0] == 6:
            debug_print(self.name, f"using EULER set position to {position}", self.INFO)
        elif position.shape[0] == 7:
            debug_print(self.name, f"using QUATERNION set position to {position}", self.INFO)
        else:
    ...
    def __del__(self):
        try:
            if hasattr(self, 'controller'):
                ...
                pass
            debug_print(self.name, f"controller deleted", self.INFO)
        except:
            debug_print(self.name, f"error occurred while deleting controller", "ERROR")
            pass
