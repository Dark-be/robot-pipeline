...
安装依赖:
    pip install rerun-sdk h5py numpy
    # cv2 可选，用于更好的颜色映射

使用示例:
    # 可视化单个文件
    python rerun_visual.py /path/to/file.hdf5
    
    # 可视化文件夹中的所有文件
    python rerun_visual.py /path/to/folder/
    
    # 保存为.rrd文件供后续查看
    python rerun_visual.py /path/to/file.hdf5 --save output.rrd
    
    # 连接到远程查看器
    python rerun_visual.py /path/to/file.hdf5 -c --addr "127.0.0.1:9876"
"""
...
try:
    ...
    debug_print("DEPENDENCY", "rerun-sdk not installed，run \"pip install rerun-sdk\"", "ERROR")
    sys.exit(1)
...
def visualize_act_format(f, verbose=False):
    """
    处理ACT格式的HDF5文件
    左右臂数据key: left_arm, slave_left_arm, master_left_arm, right_arm, slave_right_arm, master_right_arm
    相机数据key: cam_*, slave_cam_*, master_cam_*
    触觉数据key: 包含tactile, force, pressure, touch, tac, vitac, contact, haptic等关键词
    """
    if verbose:
    ...
    tactile_keywords = ['tactile', 'force', 'pressure', 'touch', 'tac', 'vitac', 'contact', 
                        'haptic', 'sensor_force', 'torque_sensor', 'depth']
        ...
        if data_format == 'act':
            ...
            for frame_idx in range(max_frames):
                ...
                if left_arm_data['joints'] is not None and len(left_arm_data['joints']) > 0:
                    rr.log(f"left_arm/joint", rr.Tensor(left_arm_data['joints'][frame_idx]))
                if left_arm_data['gripper'] is not None and len(left_arm_data['gripper']) > 0:
                    rr.log(f"left_arm/gripper", rr.Scalars(left_arm_data['gripper'][frame_idx]))
                if left_arm_data['eefort'] is not None and len(left_arm_data['eefort']) > 0:
                    rr.log(f"left_arm/eefort", rr.Tensor(left_arm_data['eefort'][frame_idx]))
                
                # 右臂数据 - 只在数据存在时记录
                if right_arm_data['joints'] is not None and len(right_arm_data['joints']) > 0:
                    rr.log(f"right_arm/joint", rr.Tensor(right_arm_data['joints'][frame_idx]))
                if right_arm_data['gripper'] is not None and len(right_arm_data['gripper']) > 0:
                    rr.log(f"right_arm/gripper", rr.Scalars(right_arm_data['gripper'][frame_idx]))
                if right_arm_data['eefort'] is not None and len(right_arm_data['eefort']) > 0:
                    rr.log(f"right_arm/eefort", rr.Tensor(right_arm_data['eefort'][frame_idx]))
                        ...
                        if frame_idx < len(cam_dataset):
                            ...
                            if image is not None:
                                ...
                                if len(image.shape) == 2:
                                    ...
                                    image = image[:, :, :3]
                                rr.log(f"cameras/{camera_name}", rr.Image(image[:,:,::-1]).compress(jpeg_quality=20))
                    ...
                    for tactile_name, tactile_dataset in tactile_data.items():
                        if frame_idx < len(tactile_dataset):
                            ...
                            if is_tactile_image_data(tactile_dataset, frame_idx):
                                ...
                                rr.log(f"tactile/{tactile_name}_heatmap", rr.Image(tactile_colored).compress(jpeg_quality=20))
                            else:
                                # 非图像格式的触觉数据，使用时间序列显示
                                debug_print("VISUALIZE", f"触觉数据 {tactile_name} 非图像格式", "DEBUG")
...
def main():
    ...
    if args.save:
        ...
        # 连接到远程查看器
        addr = "rerun+http://" + args.addr + "/proxy"
        debug_print("MAIN", f"连接到Rerun查看器: {addr}", "INFO")
